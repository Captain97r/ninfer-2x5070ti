...
int test_streaming_events() {
    ...
    // Generated text may carry U+FFFD for an ill-formed generated byte sequence
    // (docs/serving.md, "Execution behavior"); it is ordinary text on the wire.
    const Json rdelta = parse_sse(make_content_block_delta_text(1, "\xef\xbf\xbd"), &type);
    failures += check(type == "content_block_delta" &&
                          rdelta.at("delta").at("text") == "\xef\xbf\xbd",
                      "replacement character round-trips through a text_delta");
