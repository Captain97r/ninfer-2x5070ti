...
template <class Geometry, class Schedule, bool FullTokens, class Epilogue, class Output,
          ...
          class WeightScale = __nv_bfloat16>
__global__ __launch_bounds__(Schedule::kThreads, Schedule::kMinBlocksPerSm) void fp8_mma_kernel(
    ...
    const std::uint8_t* __restrict__ weight_codes, const WeightScale* __restrict__ weight_scales,
...
#pragma unroll
        for (int mma_row = 0; mma_row < Schedule::kMmaRows; ++mma_row) {
            ...
            float2 weight_scale;
            if constexpr (std::is_same_v<WeightScale, float>) {
                weight_scale = make_float2(weight_scales[parent_row0], weight_scales[parent_row1]);
            } else {
                const std::uint32_t scale_bits = load_vec<std::uint32_t>(weight_scales + parent_row0);
                weight_scale = bf16x2_bits_to_float2(scale_bits);
            }
            float value00 =
