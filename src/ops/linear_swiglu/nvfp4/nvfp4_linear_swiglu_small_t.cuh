...
// Private kernel/launch body shared by the production dispatcher and the bounded TP2
// CTA-grouping benchmark. No run-time/public tuning interface is introduced.
...
template <class Geometry, int ActiveTokens, int WarpsPerCta>
void launch_nvfp4_swiglu_small_t(const Tensor& x, const Weight& weight, Tensor& out,
                                 cudaStream_t stream) {
    constexpr int kIntermediate = Geometry::kOutputRows / 2;
    ...
    using Schedule = Nvfp4SmallTSchedule<WarpsPerCta, 1, 2, 16, ActiveTokens, 1, kActivationAccess,
