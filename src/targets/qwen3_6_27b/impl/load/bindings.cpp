...
ShardMapping shard_mapping(std::string_view object, int tp, const TextConfig& config) {
    ...
    // Vision tower: replicated on EVERY device. The tp2 vision path (phase 3A) runs the same
    // single-device encoder independently on each rank against its own full weight copy, so the
    // placement is "whole object, both devices" and nothing about it is axis-split. Prefix
    // matching is deliberate: it covers every vision object the binder can name (patch/position
    // embeddings, all 27 layers, merger) and stays correct if the tower gains tensors later.
...
ArtifactLoadPlan bind_artifact(artifact::Binder& binder, WeightsProfile weights_profile,
    ...
    if (tp > 1) {
        ...
        // Replicated placement), so a tp2 bind with --vision places the whole 282 MiB tower on
        // both devices by design; see research/phase3a-vision-memory.md for the feasibility
        // analysis that chose dual replication.
        const TextConfig config{};
...
void LoadedModelData::build_device_view(const BindingPlan& plan, int device,
    ...
    if (plan.features.vision) {
        // Dual-replicated vision (phase 3A): each rank materializes the full tower view from
        // its own arena. `device` threads the placement; a rank's view points only at its own
        // device's bytes.
